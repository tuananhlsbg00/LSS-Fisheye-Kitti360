...
def gen_dx_bx(xbound, ybound, zbound):
    ...
    nx = torch.LongTensor([(np.abs(row[1] - row[0]) / row[2]) for row in [xbound, ybound, zbound]])
...
def get_val_info(model, valloader, loss_fn, device, use_tqdm=False, counter = 0):
    model.eval()
    total_loss = 0.0
    total_intersect = 0.0
    total_union = 0
    ...
    with torch.no_grad():
        for i, batch in enumerate(loader):
            imgs, rots, trans, K, D, xi, binimgs = batch
            ...
            output_dir = f'./evaluate/Step_{counter}'
            ...
            evalimg = np.hstack((pred, binimg)).astype(np.uint8)

            cv2.imwrite(os.path.join(output_dir, f'Batch_{i}.png'), evalimg)
