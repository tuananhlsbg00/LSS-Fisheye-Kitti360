...
class Kitti360Viewer(QtWidgets.QMainWindow):
    ...
    def updateScale(self, qp):
        if not self.image.width() or not self.image.height():
            return
        # Retrieve the toolbar reliably:
        tb = self.findChild(QtWidgets.QToolBar)
        if tb is None:
            # If toolbar is gone, fallback to a default value.
            toolbar_height = 0
        else:
            toolbar_height = tb.height()
        # Horizontal offset
        self.xoff = self.bordergap
        # Vertical offset using the toolbar height
        self.yoff = toolbar_height + self.bordergap
    ...
    def drawLabelAtMouse(self, qp):
        # Nothing to do if mouse is outside of the image
        if self.mouseOutsideImage:
            return
        # Nothing to do without a mouse position
        ...
        # Safely get toolbar height; if toolbar is deleted, default to 0.
        try:
            tb_height = self.toolbar.height()
        except RuntimeError:
            tb_height = 0

        if mouse.y() - off > tb_height:
            top = mouse.y() - off
        ...
        rect.setBottomRight(QtCore.QPoint(mouse.x() + 200, mouse.y() + off))
    ...
    def getSequenceFromUser(self):
        ...
        if items:
            ...
            if ok and item:
                ...
                self.annotation2DInstance = Annotation2DInstance(os.path.join(label2DPath, sequence, 'image_00'))
